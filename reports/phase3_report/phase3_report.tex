...
combined through tuned RBF Support Vector Machines. Under repeated
stratified evaluation the best fixed teacher configuration reaches
macro-F1\,=\,\textbf{0.6566}\,$\pm$\,0.048 (best seed\,=\,0.7486), a
+0.149 absolute improvement over Phase~2's hybrid ensemble and +0.387
over the Phase~1 baseline. We then distil a validation-weighted
multi-teacher ensemble into compact EfficientAT MobileNetV3 students
(``mn10\_as'' and ``mn04\_as'') using KL-divergence distillation,
label-smoothed cross entropy, class-balanced sampling, mixup, and
SpecAugment. In the final three-seed Colab run, the 4.21\,M-parameter
\texttt{mn10\_as} student achieves macro-F1\,=\,\textbf{0.7159}\,$\pm$\,0.0403,
retaining 99.9\,\% of the teacher's 0.7168 macro-F1 while running as an
8.62\,MB INT8 model at 79.2\,ms CPU latency. The 0.72\,M-parameter
\texttt{mn04\_as} student achieves macro-F1\,=\,\textbf{0.6500}\,$\pm$\,0.0390
in a 1.65\,MB INT8 footprint, making the system deployable on commodity
edge hardware in hospitals and home settings.
\end{abstract}
...
\begin{enumerate}[nosep]
  ...
  \item A Phase~3 edge distillation pipeline that distils a
        ...
        ``mn10\_as'' and ``mn04\_as'' students with KL distillation,
        mixup, SpecAugment, class-balanced sampling, INT8 quantisation,
        and CPU latency benchmarking. The final three-seed run reaches
        0.7159\,$\pm$\,0.0403 macro-F1 for \texttt{mn10\_as} and
        0.6500\,$\pm$\,0.0390 for \texttt{mn04\_as}.
\end{enumerate}
...
\texttt{notebooks/Phase2A\_Pretrained\_Feature\_Bank.ipynb}. The final
run used three repeated stratified seeds, 20 epochs per seed, batch size
32, and early stopping on validation macro-F1. The validation-weighted
teacher ensemble reached 0.7168\,$\pm$\,0.0185 test macro-F1 across
these same splits.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{edge_actual_results.png}
  \caption{Actual Phase~3 edge distillation results. The
  \texttt{mn10\_as} student matches the multi-teacher ensemble within
  noise (99.9\,\% macro-F1 retention), while the much smaller
  \texttt{mn04\_as} student retains 90.7\,\% of the teacher score in a
  1.65\,MB INT8 footprint.}
\end{figure}

\begin{table}[H]
  \centering
  \small
  \caption{Final three-seed EfficientAT distillation results. All
  student metrics use the same repeated train/validation/test splits as
  the teacher ensemble. Latency is batch-1 inference on a 10-second
  clip.}
  \label{tab:edge}
  \begin{tabularx}{\linewidth}{l c c c c c}
    \toprule
    \textbf{Model} & \textbf{Macro-F1} & \textbf{Retention} &
    \textbf{INT8 size} & \textbf{CPU INT8} & \textbf{GPU} \\
    \midrule
    Multi-teacher ensemble & 0.7168\,$\pm$\,0.0185 & 100.0\,\% & -- & -- & -- \\
    \texttt{mn10\_as} & \textbf{0.7159\,$\pm$\,0.0403} & \textbf{99.9\,\%} &
    8.62\,MB & 79.2\,ms & 9.3\,ms \\
    \texttt{mn04\_as} & 0.6500\,$\pm$\,0.0390 & 90.7\,\% &
    1.65\,MB & 44.1\,ms & 9.1\,ms \\
    \bottomrule
  \end{tabularx}
\end{table}

\paragraph{Per-class student behaviour.}
The \texttt{mn10\_as} student reaches per-class F1 of
0.798\,$\pm$\,0.042 for hunger, 0.700\,$\pm$\,0.142 for belly pain,
0.869\,$\pm$\,0.102 for burping, 0.655\,$\pm$\,0.089 for discomfort, and
0.558\,$\pm$\,0.053 for tiredness. Its Expected Calibration Error (ECE)
is 0.0758\,$\pm$\,0.0031 and teacher/student agreement is
80.4\,$\pm$\,3.8\,\%. The smaller \texttt{mn04\_as} model reaches
90.7\,\% macro-F1 retention with 0.0878\,$\pm$\,0.0125 ECE, showing
that useful cry classification remains possible even below one million
parameters.
...
final step. The final results are stronger than the conservative target:
\texttt{mn10\_as} matches the teacher (0.7159 vs.\ 0.7168 macro-F1) while
using only a single compact audio CNN at deployment time, and
\texttt{mn04\_as} retains 90.7\,\% of teacher macro-F1 in a 1.65\,MB INT8
artifact. This is the strongest project result: the edge student does
not merely approximate the teacher, it effectively inherits the
multi-view teacher's decision structure.
...
pretrained teacher of macro-F1\,=\,0.6566\,$\pm$\,0.048 on the strict
five-class manifest. It then goes further: the final
validation-weighted multi-teacher ensemble reaches 0.7168\,$\pm$\,0.0185
macro-F1 on the edge-distillation splits, and the deployed
\texttt{mn10\_as} EfficientAT student reaches
0.7159\,$\pm$\,0.0403 macro-F1 with 99.9\,\% retention in an 8.62\,MB
INT8 artifact. The smaller \texttt{mn04\_as} model remains highly
competitive at 0.6500\,$\pm$\,0.0390 macro-F1 in only 1.65\,MB. Phase~3
therefore delivers both accuracy and deployability: Colab-runnable code,
full provenance, repeated-eval JSON outputs, INT8 artifacts, and CPU/GPU
